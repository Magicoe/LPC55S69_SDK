 ...
 *
 */
__STATIC_FORCEINLINE void hashcrypt_sha_ldm_stm_16_words(volatile uint32_t *dst, const uint32_t *src)
{
    struct tt
    {
        uint32_t a[8];
    } * ldst, *lsrc;
    ldst = (void *)(uintptr_t)dst;
    lsrc = (void *)(uintptr_t)src;
    ldst[0] = lsrc[0];
    ldst[0] = lsrc[1];
}
 ...
 *
 */
__STATIC_INLINE void hashcrypt_load_data(volatile uint32_t *dst, const uint32_t *src, size_t size)
{
...
static void hashcrypt_aes_load_userKey(HASHCRYPT_Type *base, hashcrypt_handle_t *handle)
{
    ...
    hashcrypt_load_data(&base->INDATA, &handle->keyWord[0], keySize);
}
 ...
 */
static status_t hashcrypt_sha_check_input_alg(hashcrypt_algo_t algo)
{
    if ((algo != kHASHCRYPT_Sha1) && (algo != kHASHCRYPT_Sha256))
    {
        return kStatus_InvalidArgument;
    }
    else if ((!(HASHCRYPT->CONFIG & HASHCRYPT_CONFIG_SHA512_MASK)) && (algo == kHASHCRYPT_Sha512))
    {
        return kStatus_InvalidArgument;
    }
...
static status_t hashcrypt_sha_check_input_args(HASHCRYPT_Type *base, hashcrypt_hash_ctx_t *ctx, hashcrypt_algo_t algo)
{
    ...
    if (kStatus_Success != hashcrypt_sha_check_input_alg(algo))
    {
        return kStatus_InvalidArgument;
    }
 ...
 * @return kStatus_Success if valid, kStatus_InvalidArgument otherwise.
 */
static status_t hashcrypt_sha_check_context(hashcrypt_sha_ctx_internal_t *ctxInternal)
{
    if ((NULL == ctxInternal) || (kStatus_Success != hashcrypt_sha_check_input_alg(ctxInternal->algo)))
    {
        return kStatus_InvalidArgument;
    }
    return kStatus_Success;
...
static void hashcrypt_sha_one_block(HASHCRYPT_Type *base, const uint8_t *blk)
{
    ...
    hashcrypt_sha_ldm_stm_16_words(&base->INDATA, actBlk);
}
...
status_t HASHCRYPT_SHA_Update(HASHCRYPT_Type *base, hashcrypt_hash_ctx_t *ctx, const uint8_t *input, size_t inputSize)
{
    ...
    status = hashcrypt_sha_check_context(ctxInternal);
                                  ...
                                  const uint8_t iv[16])
{
    ...
    /* load 16b iv */
    hashcrypt_load_data(&base->INDATA, (uint32_t *)iv, 16);
                                ...
                                size_t *szLeft)
{
    ...
    /* load nonce */
    hashcrypt_load_data(&base->INDATA, (uint32_t *)counter, 16);
