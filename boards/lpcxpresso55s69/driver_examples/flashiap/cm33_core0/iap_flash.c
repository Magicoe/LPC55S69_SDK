...
#define FLASH_ERASE_SIZE 0x200U
...
int main()
{
    ...
    status = FLASH_Erase(&flashInstance, TEST_DEST_ADDRESS, FLASH_ERASE_SIZE, kFLASH_ApiEraseKey);
    verify_status(status);
    PRINTF("Done!\r\n");

    /* Verify if the given flash range is successfully erased. */
    PRINTF("Calling FLASH_VerifyErase() API...\r\n");
    status = FLASH_VerifyErase(&flashInstance, TEST_DEST_ADDRESS, FLASH_ERASE_SIZE);
    verify_status(status);
    ...
    status = FLASH_Program(&flashInstance, TEST_DEST_ADDRESS, (uint8_t *)s_buffer, sizeof(s_buffer));
    verify_status(status);

    /* Verify if the given flash region is successfully programmed with given data */
    PRINTF("Calling FLASH_VerifyProgram() API...\r\n");
    status = FLASH_VerifyProgram(&flashInstance, TEST_DEST_ADDRESS, sizeof(s_buffer), (const uint8_t *)s_buffer,
    ...
    {
        s_buffer_rbc[i] = *(volatile uint32_t *)(TEST_DEST_ADDRESS + i * 4);
    ...
    PRINTF("\r\n Successfully Programmed and Verified Location 0x%x -> 0x%x \r\n", TEST_DEST_ADDRESS,
           (TEST_DEST_ADDRESS + sizeof(s_buffer)));

    /* resume the flash memory status */
    status = FLASH_Erase(&flashInstance, TEST_DEST_ADDRESS, FLASH_ERASE_SIZE, kFLASH_ApiEraseKey);
