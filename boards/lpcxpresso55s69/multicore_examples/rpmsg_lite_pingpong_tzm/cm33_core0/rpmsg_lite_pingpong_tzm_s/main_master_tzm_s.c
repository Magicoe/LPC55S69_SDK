...
#define CORE1_BOOT_ADDRESS (void *)0x30033000
...
#elif defined(__GNUC__) /* GCC, MCUXpresso */
...
int main(void)
{
    ...
    rpmsg_lite_instance_s = rpmsg_lite_master_init(rpmsg_lite_base, SH_MEM_TOTAL_SIZE, RPMSG_LITE_LINK_ID,
    ...
    msg.DATA = 0;
    rpmsg_lite_send(rpmsg_lite_instance_s, my_ept_s, REMOTE_EPT_ADDR, (char *)&msg, sizeof(THE_MESSAGE), RL_DONT_BLOCK);
    ...
    while (msg.DATA <= 50)
    {
        if (has_received)
        {
            has_received = 0;
            msg.DATA++;
            rpmsg_lite_send(rpmsg_lite_instance_s, my_ept_s, REMOTE_EPT_ADDR, (char *)&msg, sizeof(THE_MESSAGE),
                            RL_DONT_BLOCK);
        }
    }
